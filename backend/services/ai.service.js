...
export const generateLeadSummary = async (lead) => {
  const prompt = `You are an expert B2B sales analyst for a CRM called ClientFlow CRM. Analyse the following sales lead and produce a concise assessment.
...
Return JSON only with a compelling subject line and a complete email body. Use line breaks (\\n) in the body. Keep it under 180 words. Sign off as ${
    sender?.name || "the ClientFlow CRM team"
  }.`;
